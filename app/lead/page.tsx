import { Footer } from "@/components/Footer";
import { Header } from "@/components/Header";
import { LeadForm } from "@/components/LeadForm";

type LeadPageProps = {
  searchParams?: {
    role?: string;
    teacher?: string;
    subject?: string;
    level?: string;
  };
};

export default function LeadPage({ searchParams }: LeadPageProps) {
  const initialRole = searchParams?.role === "tutor" ? "tutor" : "student";

  return (
    <>
      <Header
        showSectionLinks={false}
        navItems={[
          { href: "/", label: "Главная" },
          { href: "/teachers", label: "Преподаватели" },
          { href: "/assessment", label: "Тестирование" },
          { href: "/for-tutors", label: "Для преподавателей" }
        ]}
        navLabel="Навигация по заявке"
        secondaryAction={{ label: "Войти", href: "/login" }}
        primaryAction={{ label: "Регистрация", href: initialRole === "tutor" ? "/signup?role=tutor" : "/signup?role=student" }}
      />

      <main className="relative min-h-screen overflow-hidden pb-16 pt-28 sm:pt-32">
        <div className="pointer-events-none absolute inset-0 -z-10 bg-[radial-gradient(circle_at_20%_10%,rgba(116,76,255,0.15),transparent_35%),radial-gradient(circle_at_80%_20%,rgba(185,250,119,0.22),transparent_32%),linear-gradient(to_bottom,#fcfcff,white)]" />
        <div className="mx-auto w-full max-w-7xl px-4 sm:px-6 lg:px-8">
          <LeadForm
            initialRole={initialRole}
            selectedTeacher={searchParams?.teacher}
            subjectHint={searchParams?.subject}
            levelHint={searchParams?.level}
          />
        </div>
      </main>

      <Footer />
    </>
  );
}
